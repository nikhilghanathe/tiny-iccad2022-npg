...
class IEGM_DataSET():
    ...
    def __getitem__(self, idx):
        ...
        IEGM_seg = txt_to_numpy(text_path, self.size).reshape(self.size, 1, 1)
        label = int(self.names_list[idx].split(' ')[1])
        sample = {'IEGM_seg': IEGM_seg, 'label': label}
        # sample = IEGM_seg, label
        return sample
...
def custom_generator_train(gen):
    ...
    while True:
        ...
        for (x, y) in gen:
            ...
            if BS<BATCH_SIZE:
              x_0, x_1 = x, y
              dummy_x_tensor = tf.constant(0, dtype='float32', shape=[BATCH_SIZE-BS, 1250,1,1])
              dummy_y_tensor =  tf.zeros(shape=[BATCH_SIZE-BS,num_classes],  dtype='float32')
